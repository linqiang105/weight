...
//文件路径
procedure Calc_CRC32(filename: string; var CRC32: Dword);
//字符函数
function Get_Crc32(S: string; Seed: Longint): string;
  ...
  begin
    repeat
      ...
      for i := 1 to BytesRead do
        CRC32 := (CRC32 shr 8) xor CRCTable32[Buffer[i]] xor (CRC32 and $000000FF); //取反运算
    until BytesRead = 0;
  end;
  CloseFile(f);
  CRC32 := not CRC32;
...
//CRC   文件校验
function JesionCRC(FileStr: pchar): pchar; stdcall;
var
  CRC: Dword;
begin
  Calc_CRC32(FileStr, CRC);
  if CRC <> 0 then
    result := pchar(IntToHex(CRC, 6));
end;
...
begin
  ...
  for Count := 1 to Length(S) do
    CrcVal := CRCTable32[byte(CrcVal xor Dword(ord(S[Count])))] xor ((CrcVal shr 8) and $00FFFFFF); //取反运算
  result := IntToHex(not (CrcVal), 2);
end;
